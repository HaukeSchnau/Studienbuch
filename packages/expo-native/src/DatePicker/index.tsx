 ...
 * @hidden
 */
export function transformDateTimePickerProps(
  props: DateTimePickerProps,
): NativeDatePickerProps {
  const { iosVariant, androidVariant, ...rest } = props;
  const { minWidth, minHeight, ...restStyle } =
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-condition -- Not sure the types are correct
  ...
  return {
    ...
    style: [
      restStyle,
      { minWidth: parsedMinWidth, minHeight: parsedMinHeight },
    ],
  };
}

const DatePickerNativeView: React.ComponentType<NativeDatePickerProps> =
  requireNativeView("ExpoUI", "DateTimePickerView");
