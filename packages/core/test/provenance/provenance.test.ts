...
describe("source reconciliation", () => {
  it("retains provider-scoped identity and deterministically ignores duplicates and stale values", () => {
    const current = sourcedValue(observation("Mathematics", 2, { code: "MA", name: "Math" }));
    const duplicate = reconcileIncoming(
      current,
      observation("Mathematics", 2, { name: "Math", code: "MA" }),
    );
    expect(duplicate._tag).toBe("Duplicate");

    const stale = reconcileIncoming(current, observation("Old mathematics", 1));
    expect(stale._tag).toBe("Stale");
    expect(stale.value).toBe(current);

    const otherProvider = DataSource.make({
      id: source.id,
      provider: NonEmptyText.make("Another provider"),
    });
    const renamedProvider = reconcileIncoming(current, {
  ...
  it.effect("preserves and relinquishes a field-level user override", () =>
    Effect.gen(function* () {
      const current = applyOverride(sourcedValue(observation("Room 1", 1)), {
        value: "My room",
        changedBy: actor,
        changedAt: DateTime.makeUnsafe("2026-08-10T12:00:00Z"),
      });
      const result = reconcileIncoming(current, observation("Room 2", 2));
      ...
      const relinquished = yield* relinquishOverride(result.value);
  ...
  it("distinguishes raw-source churn from an effective value change", () => {
    const current = sourcedValue(observation("Mathematics", 1, { label: "Math" }));
    const result = reconcileIncoming(
      current,
  ...
  it.effect("distinguishes partial feeds from complete-feed deletion", () =>
    Effect.gen(function* () {
      const current = applyOverride(sourcedValue(observation("Imported", 1)), {
        value: "Manual",
        changedBy: actor,
        changedAt: DateTime.makeUnsafe("2026-08-10T12:00:00Z"),
      });
      const deletion = { ...stamp(2), completeness: "Partial" as const };
      const partial = reconcileSourceDeletion(current, deletion);
      assert.deepEqual(partial, { _tag: "Retained", value: current, reason: "PartialFeed" });

      const complete = reconcileSourceDeletion(current, { ...deletion, completeness: "Complete" });
      ...
      const refusal = yield* Effect.flip(relinquishOverride(complete.value));
      assert.strictEqual(refusal._tag, "Provenance.OverrideRelinquishmentRefused");
...
describe("subject resolution", () => {
  ...
  it("keeps raw labels across exact, inferred, ambiguous, and unknown outcomes", () => {
    expect(resolveSubject("  Informatik  ", catalog)).toEqual({
      _tag: "Exact",
      subjectId: SubjectId.make("informatics"),
      rawLabel: "  Informatik  ",
    });
    expect(resolveSubject("IF23", catalog)).toEqual({
      _tag: "Inferred",
      subjectId: SubjectId.make("informatics"),
      rawLabel: "IF23",
      rule: "CourseCodePrefix",
    });
    expect(resolveSubject("PoWi", catalog)).toMatchObject({
      _tag: "Ambiguous",
      rawLabel: "PoWi",
      candidateSubjectIds: [SubjectId.make("social-science-a"), SubjectId.make("social-science-b")],
    });
    expect(resolveSubject("Mystery Lab", catalog)).toEqual({
      _tag: "Unknown",
