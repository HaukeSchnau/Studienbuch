...
- Full device-level lifecycle validation is still missing (real AppState/network transitions with queued offline writes).
...
1. Add device/E2E lifecycle sync scenarios (foreground/background/offline transitions with replay verification) for `@stu/app-mobile`.
