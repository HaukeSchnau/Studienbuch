...
- `PR-08: Repository Dedup Track B (Student/Sensitive Repos)` (slice 4; holiday/student mapping plus absence/grade helper dedup)
  - Changes landed in:
  ...
  - Verification run:
    ...
    - `bun test packages/lib/src/repository-logic/grade.test.ts` (pass)
    - `bun test packages/db/src/applicators.integration.test.ts` (pass)
    - `bun test packages/student/src/applicators.integration.test.ts` (pass)
- Next queued:
- `PR-08`: continue low-risk serializer/helper dedup for student-sensitive repos (next: timetable/person read-shape helpers before larger write-path merges)
