...
class StudienbuchInlineBadgeNativeView: ExpoView {
  private let label = UILabel()
  private var accent = UIColor(red: 0.54, green: 0.36, blue: 0.96, alpha: 1)
  ...
  var accentColor: String = "#8a5cf6" {
    didSet {
      accent = UIColor(inlineHexString: accentColor) ?? UIColor(red: 0.54, green: 0.36, blue: 0.96, alpha: 1)
      updateAppearance()
    }
  }
...
private extension UIColor {
  convenience init?(inlineHexString: String) {
    var hex = inlineHexString.trimmingCharacters(in: .whitespacesAndNewlines)
    if hex.hasPrefix("#") {
      hex.removeFirst()
    }
