...
Set `MOBILE_E2E_AGENT_DEVICE` and `MOBILE_E2E_ARGENT_DEVICE` when auto-detection could select
different devices. Android normally needs both because agent-device selects the AVD name while
Argent selects its ADB serial. `MOBILE_E2E_DEVICE` is shorthand when both runners accept one value.
The Argent selector is required when a later job follows Argent (including
`MOBILE_E2E_ORDER=argent-first`) so the harness can release only that device's automation services
before the next runner. Runtime reports capture both selectors, versions, order, exit codes, and
wall-clock duration; use repository history and agent-thread history to compare authoring effort,
repair effort, and churn.
...
The optional environment variables are `MOBILE_E2E_DEVICE`, `MOBILE_E2E_AGENT_DEVICE`,
`MOBILE_E2E_ARGENT_DEVICE`, `MOBILE_E2E_ORDER` (`agent-device-first` or `argent-first`), and
`MOBILE_E2E_TIMEOUT_MS`.
