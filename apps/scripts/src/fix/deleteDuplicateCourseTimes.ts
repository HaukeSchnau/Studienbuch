...
export const deleteDuplicateCourseTimes = async () => {
  ...
  for (const course of courses) {
    ...
    for (const time of times) {
      const duplicateTimes = times.filter(
        (t) =>
          t.weeks === time.weeks &&
          t.start === time.start &&
    ...
    if (timesToDelete.length > 0) {
      console.log(
        `Deleting ${timesToDelete.length} duplicate times for course ${course.id}`,
      );
      await db.delete(CourseTime).where(
        inArray(
          CourseTime.id,
          timesToDelete.map((t) => t.id),
        ),
      );
    }
  }
